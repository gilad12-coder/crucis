...
def _add_fit_parser(subs) -> None:
    ...
    p.add_argument(_OBJECTIVE_POS, nargs="?", default=None, help="Path to objective YAML file")
    p.add_argument(
        _OBJECTIVE_FLAG,
        dest="objective_flag",
...
def _add_doctor_parser(subs) -> None:
    ...
    p.add_argument(_JSON_OPTION, action=_STORE_TRUE, help="Print machine-readable JSON output")
...
def main(argv: list[str] | None = None) -> None:
    ...
    handlers = {
        ...
        "evaluate": _handle_evaluate_command,
...
def _handle_fit_command(args: argparse.Namespace) -> None:
    ...
    """
    raw_path = args.objective_path or getattr(args, "objective_flag", None)
    if not raw_path:
        display_error("No objective file specified. Usage: crucis fit <objective.yaml>")
        raise SystemExit(2)
...
def _handle_evaluate_command(args: argparse.Namespace) -> None:
    ...
    Args:
        args: Parsed CLI arguments object.
    """
    raw_path = getattr(args, _OBJECTIVE_POS, None) or getattr(args, "objective", None)
    if not raw_path:
        display_error("No objective file specified. Usage: crucis evaluate <objective.yaml>")
        raise SystemExit(2)
    objective_path = Path(raw_path)
...
def _try_agent_onboarding(workspace: Path, args: argparse.Namespace) -> bool:
    ...
    if shutil.which(agent) is None:
        print(f"Agent '{agent}' not found on PATH. Using static templates.")
        return False

    success = run_agent_onboarding(workspace, agent, model)
    if not success:
        print("Agent onboarding did not complete. Using static templates.")
        return False

    if not validate_onboarding_output(workspace):
        print("Agent output is incomplete or invalid. Using static templates.")
        return False
...
def run_doctor_command(args: argparse.Namespace) -> None:
    ...
    report = run_doctor(
        ...
        profiles_path=Path(args.profiles) if getattr(args, "profiles", None) else None,
